...
router = APIRouter(prefix="/api/zapier")
...
async def forward_event(request: Request):
    ...
    resp = supabase.table("webhook_subscriptions").select("target_url").eq("user_id", user_id).eq("service", "zapier").maybe_single().execute()
